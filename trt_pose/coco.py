...
def coco_annotations_to_tensors(
    coco_annotations, image_shape, parts, topology, max_count=100
):
...
def convert_dir_to_bmp(output_dir, input_dir):
    ...
    for f in files:
        new_path = os.path.join(
            output_dir, os.path.splitext(os.path.basename(f))[0] + ".bmp"
        )
...
def transform_peaks(counts, peaks, quad):
    ...
    for c in range(C):
        count = int(counts[c])
        newpeaks[c][0:count] = transform_points_xy(newpeaks[c][0:count][:, ::-1], quad)[
            :, ::-1
        ]
...
class CocoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = coco_annotations_to_mask_bbox(
            self.samples[idx]["anns"], (im["height"], im["width"])
        )
        ...
        shiftx = (
            float(torch.rand(1)) * (self.random_translate[1] - self.random_translate[0])
            + self.random_translate[0]
        )
        shifty = (
            float(torch.rand(1)) * (self.random_translate[1] - self.random_translate[0])
            + self.random_translate[0]
        ...
        scale = (
            float(torch.rand(1)) * (self.random_scale[1] - self.random_scale[0])
            + self.random_scale[0]
        )
        angle = (
            float(torch.rand(1)) * (self.random_angle[1] - self.random_angle[0])
            + self.random_angle[0]
        )
        ...
        cmap = trt_pose.plugins.generate_cmap(
            counts,
            peaks,
            self.target_shape[0],
            self.target_shape[1],
...
class CocoHumanPoseEval(object):
    def __init__(
        self, images_dir, annotation_file, image_shape, keep_aspect_ratio=False
    ):
